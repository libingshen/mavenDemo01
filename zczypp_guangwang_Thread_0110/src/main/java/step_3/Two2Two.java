...
public class Two2Two implements Runnable{
	...
	public void run(){
		...
		String logPath = "C:"+File.separator+"GWlog"+File.separator+"光网匹配"+File.separator+logFileName+".txt";
		...
		try {
			...
			for(Map<String,String> m : location){
				//获取所有index
				List<Map<String,String>> list = new getIndex2nd().getIndexList(conn,m.get("zc_id"),m.get("zhanzhi_name"));//zc_id='光网OLT'，zhanzhi_name为站址名
	...
	public static void main(String[] args) throws Exception {
		...
		new Thread(new Two2Two(DbUtil.getConn(),1,2,countDownLatch_1)).start();
	}
}
